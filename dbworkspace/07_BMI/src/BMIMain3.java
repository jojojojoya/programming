...
public class BMIMain3 {
	...
	private void selectByStatus() {
		System.out.println("1. 저체중 2. 정상  3. 과체중 \n 4. 비만 5. 고도비만");
		int pick = sc.nextInt();
		String sql = "select * from bmi_test where b_status = ?";

		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, statusTbl[pick]);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				System.out.printf("%d]  %s / %.1f / %.1f /%.1f / %s", rs.getInt(1), rs.getString(2), rs.getDouble(3),
