...
public interface UserMyPageMapper {
    ...
    // 유저 관련 DB를 리스팅
    @Select("SELECT * FROM main_user WHERE user_id = #{user_id}")
    List<UserMyPageVO> getUserById(@Param("user_id") String user_id);

    // 패스워드를 확인하기 위해 불러오기 (대조)
    @Select("SELECT user_password FROM main_user WHERE user_id = #{user_id}")
...
""")
    ...
    // 챗봇 대화 요약내역 조회
    @Select("SELECT chat_summary FROM main_chat WHERE user_id = #{user_id}")
...
""")
    ...
    @Select("SELECT * FROM main_counseling_reservation")
